...
> Last updated: 2026-06-14 | Android version: v1.17.0-rc2 | Protocol: mobile-gateway.v1
...
Implemented through v1.17.0-rc2:
...
- Android instrumented Compose smoke test that drives the pairing UI against an in-process mock gateway for `/mobile/health` and `/mobile/pair/claim`.
...
Gateway events parsed by Android v1.17.0-rc2:
...
- `./gradlew :app:assembleDebugAndroidTest` verifies the instrumented Compose pairing smoke test compiles for device execution.
- Version `1.17.0-rc2` is visible in Gradle metadata and sent through gateway request headers.
