...
- Preserve multiple paired Mac credentials and let the user switch the active remote computer from the top of the home screen.
...
- Strict live acceptance may auto-approve pending tools only when they require neither an answer nor typed confirmation; it must not bypass danger-confirm or ask-user gates.
...
- The connected-device smoke runner can auto-discover the default WillDeep Mac runtime desktop-token file under `~/Library/Application Support/com.willdeep.app/MobileGateway/desktop-token` and use `http://127.0.0.1:8877` for desktop-authenticated pairing rotation when no live payload or explicit desktop gateway variables are provided.
...
- The final live acceptance wrapper adds a unique run marker to the default workspace-edit request so repeated runs produce observable file changes.
- The connected-device smoke runner filters live-payload instrumentation to the live Mac gateway test and records `host_target_file` fallback evidence when the requested Mac workspace file changes after the Android-originated command but Android misses the exact target-file event.
...
Implemented in v1.24.0-rc1:

- Encrypted multi-Mac credential storage with automatic migration from the legacy single credential and stable relay-room identity.
- A top-level remote computer selector with last-response status and scan, switch, disconnect, and remove actions.
- Transport/app state separation: WebSocket `onOpen` only establishes the server channel; Android displays `Connected` only after a parsed Mac-originated snapshot, acknowledgement, session, message, approval, workspace, capability, job, file, or worktree event.
- Five-second `session.list` polling with a twenty-second Mac App response threshold. During the grace period Android retains the last truthful status; after the threshold it shows the reconnecting notice while keeping cached sessions visible.
...
Gateway events parsed by Android v1.17.0-rc52:
...
- The live acceptance wrapper also records a Mac Gateway preflight section so missing Settings enablement, missing desktop-token file, and unreachable `127.0.0.1:8877/mobile/health` are visible before the Android instrumentation step.
...
- Version `1.25.0-rc1` is visible in Gradle metadata and sent through gateway request headers.
