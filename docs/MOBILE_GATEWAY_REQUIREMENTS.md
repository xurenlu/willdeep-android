...
> Last updated: 2026-06-14 | Android version: v1.17.0-rc35 | Protocol: mobile-gateway.v1
...
Implemented through v1.17.0-rc35:
...
- The connected-device smoke runner writes `next_actions` into JSON/Markdown reports when live payloads, Android devices, health preflight, or pairing windows still need attention.
...
Gateway events parsed by Android v1.17.0-rc35:
...
- Connected smoke reports include `next_actions` to make no-device, no-payload, and skipped-preflight results immediately actionable.
...
- Version `1.17.0-rc35` is visible in Gradle metadata and sent through gateway request headers.
